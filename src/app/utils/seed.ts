...
async function seedFirstOperator() {
  ...
  await prisma.user.create({
    data: {
      ...
      role: 'POWER_OPERATOR',
      emailVerified: true,
      operatorProfile: { create: { employeeId: 'OPS-0001', designation: 'Control Room Operator', shift: 'MORNING' as any } },
    },
  });
...
async function seedDemoTechnician() {
  ...
  const operator = await prisma.user.findFirst({ where: { role: 'POWER_OPERATOR', isDeleted: false } });
  ...
  console.log(`👷 Demo technician created (by the seeded operator): ${config.seed.technician.email}`);
}
...
export async function seed() {
  const { zone, area } = await seedDemoGrid();
