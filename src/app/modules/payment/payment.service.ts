...
import { CheckoutInput } from '@modules/payment/payment.interface';
...
export const createCheckoutSession = async (customer: User, input: CheckoutInput) => {
  ...
  if (isPriority) {
    ...
    if (!report || report.customerId !== customer.id) throw new AppError(404, 'Outage report not found (must be your own)');
...
export const completePayment = async (paymentId: string, stripePaymentIntentId: string | null) => {
  await prisma.$transaction(async (tx) => {
    ...
    if (payment.type === 'PRIORITY_RESTORATION' && payment.outageReportId) {
      await tx.outageReport.update({ where: { id: payment.outageReportId }, data: { isPriority: true } });
    }
