...
export async function register(input: { name: string; email: string; password: string; category?: string; areaId?: string }) {
...
export async function refreshTokens(refreshToken?: string) {
  ...
  if (!user || user.isDeleted || user.status === 'BLOCKED') throw new AppError(401, 'Session is no longer valid');
